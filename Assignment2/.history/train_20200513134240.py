...
def train(data, epochs=100, batch_size=64, lr=1e-4, early_stopping=False):
    ...
    W = xavier_normal_(torch.empty(source_dim, target_dim)).requires_grad_(True) 
    ...
    for epoch in range(epochs):
        ...
        for proc, i in enumerate(indices):
            ...
            if proc%100==0:
                print("train set loss: " + str(np.mean(batch_loss)) + ", iter: " + str(proc) + '/' + str(len(indices)))
                batch_loss = []
