...
def Task(modeltask, model, tokenizer, embeddings, W, k=0.3):
    ...
    for sentence in source_embeds:
        ...
        predicted_tokens = Tensor([torch.argmax(pred[0][0][i]) for i in mask_indices]).type(torch.long).to(device)

        loss = 0
        loss_func = torch.nn.CosineSimilarity()
        for i, token in enumerate(predicted_tokens):
            pred_embed = model(token)[-1][0][0][1]
            original_embed = proj_t_embeds[mask_indices[i]]

            loss_temp = loss_func(pred_embed, original_embed)
            print(loss_temp)
            loss += loss_temp
            total_preds += 1
