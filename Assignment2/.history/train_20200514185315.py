...
def train(data, epochs=100, batch_size=64, lr=1e-4, early_stopping=False, constraint=False):
    ...
    for epoch in range(epochs):
        ...
        for i in range(batch_length):
            ...
            if constraint:
                u, s, v = torch.svd(torch.matmul(features, labels))
                W_constr = torch.matmul(v, u)
                loss = loss_func(preds, labels) + lambda_c*loss_func(W, W_constr)
...
if __name__ == "__main__":
    ...
    W = train(data, batch_size=64, early_stopping=True)