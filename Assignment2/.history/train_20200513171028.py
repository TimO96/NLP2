...
def train(data, epochs=100, batch_size=64, lr=1e-4, early_stopping=False):
    ...
    for epoch in range(epochs):
        ...
        batch_length = int(len(train_data[0])/batch_size)+1

        for i in range(batch_length):
            index = indices[i*batch_length:(i+1)*batch_length]
            features = torch.stack([train_data[0][i] for i in index], dim=0)
            labels = torch.stack([train_data[1][i] for i in index], dim=0)
        ...
        with torch.no_grad():
            ...
            batch_length = int(len(dev_data[0])/batch_size)+1
            batch_loss = []

            for i in range(batch_length):
                index = indices[i*batch_length:(i+1)*batch_length]
                features = dev_data[0][index]
                labels = dev_data[1][index]
    ...
    with torch.no_grad():
        ...
        batch_length = int(len(test_data[0])/batch_size)+1
        batch_loss = []

        for i in indices:
            index = indices[i*batch_length:(i+1)*batch_length]
            features = test_data[0][index]
            labels = test_data[1][index]
...
if __name__ == "__main__":
    ...
    W = train(data, early_stopping=True)